...
@implementation BoxLoginBuilderMac
...
- (void)startLoginProcess {
	...
	if (!self.ticketOperation.ticket || self.ticketOperation.ticket == @"") {
        if (_delegate && [_delegate respondsToSelector:@selector(startActivityIndicator)]) {
...
- (void)operation:(BoxOperation *)op didCompleteForPath:(NSString *)path response:(BoxOperationResponse)response {
	...
	// what we do depends on which operation completed
		...
		dispatch_async(dispatch_get_main_queue(), ^{
			...
			[[_webView mainFrame] loadRequest:[NSURLRequest requestWithURL:self.ticketOperation.authenticationURL]];
		});
		...
		dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (_delegate && [_delegate respondsToSelector:@selector(stopActivityIndicator)]) {
...
- (void)webView:(WebView *)sender didFinishLoadForFrame:(WebFrame *)frame {
    ...
    // TODO: this is a hack so we can sign in
